...
export function calc_note_label(s:system, n:nodecoord, dimension_a_is_large:boolean, note_labels_letters:boolean):string{
    let diatonic_step = (n.aa + n.bb + 50*(s.a + s.b)) % (s.a + s.b);
    let accidental_identifier = Math.floor((n.aa*s.b - n.bb*s.a-2)/ (s.a + s.b)+1) * (dimension_a_is_large ? -1 : 1);
    let accidental = accidental_identifier>0?'♯'.repeat(accidental_identifier):accidental_identifier<0?'♭'.repeat(-accidental_identifier):'';
