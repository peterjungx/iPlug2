<script lang='ts'>
    ...
    import {apply_lattice_transform, node_at_coord, calc_generator_coord, calc_note_label} from './lattice_math';
    ...
    export let highlighted_midi_notes:number[] = [];
    export let last_midi_played_trigger:number = -1;
    export let depth = 3;
    ...
    function generate_node_coords_for_window(window_width:number, window_offset:number, s:system, g:nodecoord, depth:number){
    ...
    $: generate_node_coords_for_window(window_width, window_offset, s, generator_coord, depth);
    ...
    function update(
        ...
        base_freq:number,
        last_midi_played_trigger:number
        ...
        _nodeinfos = nodes.map((n,i) => {
            ...
            let note_label = calc_note_label(s, n.c, dimension_a_is_large, note_labels_letters)
            ...
            let is_highlighted = highlighted_midi_notes.includes(midi);
            return {n, midi, key_color, note_label, midi_label, pitch_freq, pitch_ct, pitch_label, on_scale:n.on_scale, is_root, is_highlighted};
        });
        
    }
    $: update(s, edge_length, color, affine_t, window_width, window_offset, tuning_data.base_freq, last_midi_played_trigger);
