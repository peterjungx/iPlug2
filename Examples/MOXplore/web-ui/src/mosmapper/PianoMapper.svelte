<script lang='ts'>
    ...
    let oct_below = 2;
    let oct_above = 2;
    let scale = calc_scale(display_data.s, mode-1, oct_below, oct_above);
    let scale_base = calc_scale(display_data.s, mode-1);
    let cpa_slider_min = Math.atan2(display_data.s_target.a, display_data.s_target.b);
    let cpa_slider_max = Math.atan2(display_data.s_target.a, display_data.s_target.b)+Math.PI/2;
    
    let start_midi = 60;
    let playing_midi_notes:number[] = [];
    let last_midi_played_trigger:number = -1;
    ...
    globalThis.SCMFD = (ctrlTag: number, msgTag: number, dataSize: number, msg: string) => {
        if (ctrlTag === 0) {
            ...
            // Create a single Int32Array view for all three integers
            const intView = new Int32Array(bytes.buffer, 0, 4);
            let [controlTag, nChans, chanOffset, midi_note] = intView;
            last_midi_played_trigger = midi_note;

            //console.log('midi_note', midi_note)

            if (midi_note < -858993664+256){
                midi_note += 858993664;
            }

            if (midi_note < 128){
                let idx = playing_midi_notes.indexOf(midi_note);
                if (idx === -1){
                    playing_midi_notes.push(midi_note);
                }
            }else{
                let idx = playing_midi_notes.indexOf(midi_note - 128);
                if (idx > -1){
                    playing_midi_notes.splice(idx, 1);
                }
            }
            test_msg = playing_midi_notes.join(', ');
            playing_midi_notes = playing_midi_notes;

            //console.log('SCMFD', ctrlTag, msgTag, dataSize, bytes, controlTag, nChans, chanOffset);

        }
    };
    $: playing_nodecoords = playing_midi_notes.map((n) => {return nodes_for_midi_in_strip[n].c});
    //$: console.log('playing_midi_notes', playing_midi_notes)
    ...
    //}
    function update_mode(w_offset:number, w_width:number, s:system){
        if (w_offset > w_width){
            window_offset = w_width;
        }
        let min_node_offset_mode = 1 - mode;
        let max_node_offset_mode = s.a+s.b - mode;
        if (min_node_offset_mode <= -w_offset){
            mode --;
            update_system(s, mode)
        }if (max_node_offset_mode > w_width - w_offset){
            mode ++;
            update_system(s, mode)
        }
    }
    $: update_mode(Math.round(window_offset), Math.round(window_width), display_data.s);
    ...
    function update_system(s:system, mode:number){
        display_data.s_tune.a = display_data.s.a = display_data.s_target.a = s.a;
        display_data.s_tune.b = display_data.s.b = display_data.s_target.b = s.b;
        cpa_slider_min = Math.atan2(display_data.s_target.a, display_data.s_target.b) + 1e-6;
        cpa_slider_max = Math.atan2(display_data.s_target.a, display_data.s_target.b)+Math.PI/2 - 1e-6;
    ...
    let a_scale = x_scale_factor*Math.round(window_width)/12/Math.sqrt(display_data.s_target.b**2 + display_data.s_target.a**2);
    ...
    $: update_a_scale(display_data.s_target, x_scale_factor, Math.round(window_width));
    ...
    $:a_scale_y = y_scale_factor/(x_scale_factor*(Math.round(window_width)/12)**2)* (display_data.s_target.a**2 + display_data.s_target.b**2) / display_data.edge_length;
    $: a_dx = 0;
    $: a_dy = -window_offset/(Math.round(window_width)/12)*y_scale_factor;
    ...
    function update_base_tune(base_tune:number){
        tuning_data.base_freq = 440*2**(base_tune-.75);
    }
    ...
    $: update_nodes_for_midi_in_strip(display_data.s, tuning_data.tuning, 1-Math.round(window_offset), Math.round(window_width)-Math.round(window_offset), start_midi);
...
<InfoDisplay
    s_a={display_data.s.a}
    s_b={display_data.s.b}
    mode={mode}
    scale_base={scale_base}
    tuning_base_freq={tuning_data.base_freq}
    tuning={tuning_data.tuning}
    playing_nodecoords={playing_nodecoords}
/>

<div class="container" bind:clientWidth={w} bind:clientHeight={h}>
    <svg width="100%" height="100%" viewBox="{-centerX} {-centerY-7} {w} {h}" xmlns="http://www.w3.org/2000/svg">
        ...
        <rect x={-centerX} y={-centerY-7} width={w} height={centerY-y_scale_factor*(12+0.5)} fill="black" opacity="0.4"/>
        ...
        <WindowLattice 
            ...
            oct_below={oct_below}
            octave={octave}
            highlighted_midi_notes={playing_midi_notes}
            last_midi_played_trigger={last_midi_played_trigger}
            depth={depth}
        />
