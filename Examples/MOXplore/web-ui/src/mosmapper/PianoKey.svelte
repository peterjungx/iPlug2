<script lang="ts">
    ...
    let white_shorter_than_real_factor = .9;
    let black_font_size_reduction_factor = 0.8;
    $: key_id = (midi_note_number + 144) % 12;
    $: bh = 42 * height_factor;
    $: wh = 68 * height_factor * white_shorter_than_real_factor;
    const br = 3.5;
    const wb = 12;
...
<g transform="scale({scale})">
    <path 
        ...
        fill="{color}" 
        opacity="{opacity}" 
        stroke={is_highlighted?"red":is_root?"#FFB319":"none"} 
        stroke-width={2*margin}
    />
    <text 
        x="{wcx}" 
        y="{color=='white'?wh-3:bh-3}" 
        fill="{color=="white"?"black":"white"}" 
        font-size="{20/scale*(color=="white"?1:black_font_size_reduction_factor)}" 
        text-anchor="middle" 
