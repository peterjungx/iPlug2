<script lang='ts'>
    ...
    function update_keys(nodeinfos:nodeinfo[], height:number, root_midi:number) {
        ...
        height_factor = 1.1 * height/68/scale;
        ...
        for (let i=0; i<nodeinfos.length; i++) {
            ...
            keys.push({x_pitch, y_pitch, x_eq, nodeinfo:ni, pitch_label:ni.pitch_label});
        }

        let ks = 0
        keys.forEach(k => ks += k.nodeinfo.pitch_freq);
        //console.log('update_keys', ks, keys.filter(k => k.nodeinfo.midi==61)[0].nodeinfo.pitch_label);
...
{#each _keys as k}
    ...
    <g transform="translate({k.x_eq},{y_offset})">
        <PianoKey
            ...
            pitch_label={k.pitch_label}
            is_root={k.nodeinfo.is_root}
            is_highlighted={k.nodeinfo.is_highlighted}
        />
    </g>
    <path 
        ...
        fill="none" 
        stroke={k.nodeinfo.is_highlighted?"red":k.nodeinfo.is_root?'#FFB319':k.nodeinfo.key_color}
        stroke-width={k.nodeinfo.is_highlighted?3:2}
        opacity={k.nodeinfo.on_scale?1:0.5}
    /><!--
    <circle 
        cx="{k.x_eq}" 
    ...
    <circle 
        ...
        fill={k.nodeinfo.key_color} 
        opacity={k.nodeinfo.on_scale?1:0.5} 
        stroke={k.nodeinfo.is_highlighted?"red":"none"}
        stroke-width="5"
    />    
