<script lang="ts">
  ...
  function calculateValueArcPath(angle:number) {
  ...
  function externalChangeOfValue(newValue: number) {
    updateValue(newValue);
  }
  $:externalChangeOfValue(value);
  ...
  function startDrag(e: MouseEvent | TouchEvent) {
    ...
    function onMove(e: MouseEvent | TouchEvent) {
      ...
      const valueChange = deltaY * (maxValue - minValue) / (100 * fineness);
  ...
  function onWheel(e: WheelEvent) {
    ...
    const valueChange = delta * (maxValue - minValue) / (100 * fineness);
...
<div class="container">
  ...
  <div class="value">{currentValue.toFixed(precision)} {units}</div>
</div>
