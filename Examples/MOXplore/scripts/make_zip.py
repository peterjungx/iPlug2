...
def  main():
  ...
  installer = "\installer\MOXplore Installer.exe"
   
  if demo:
    installer = "\installer\MOXplore Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\MOXplore manual.pdf" 
  ]
  ...
  ZIPNAME = "MOXplore-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "MOXplore-v" + FULLVERSIONSTR + "-win-demo.zip"
