...
MOXplore::MOXplore(const InstanceInfo& info)
  : Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  std::cout << "init values for params" << " base_tune " << base_tune_last << " octave_exp " << octave_exp_last << " g " << g_last << " period " << period_last << " offset " << offset_last
            << " depth " << depth_last << "\n";
...
MOXplore::~MOXplore() { MTS_DeregisterMaster(); }
...
void MOXplore::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void MOXplore::ProcessMidiMsg(const IMidiMsg& msg)
{
  TRACE;
